var app = new function() {
    ...
    this.mostrarPasajes = function() {
      ...
      if (this.vuelos.length > 0) {
        for (i = 0; i < this.vuelos.length; i++) {
          ...
          data += '<tr>';
          data += '<td>Destino # '+ (i+1) + ' ORIGEN: ' + this.vuelos[i].origen + ', DESTINO: ' + this.vuelos[i].destino + ', SALIDA: ' + this.vuelos[i].hora.toDateString() + " " + hora + ":" + minutos + '</td>';
    ...
    this.Reservar = function (item) {
      ...
      document.getElementById('datosvuelo').innerHTML = "Destino # " + (item + 1) + ":<br>ORIGEN: " + this.vuelos[item].origen + '<br>DESTINO: ' + this.vuelos[item].destino + '<br>SALIDA: ' + this.vuelos[item].hora.toDateString() + " " + hora + ":" + minutos + '<br>PRECIO BASE: $' + this.vuelos[item].costobase + '<br>PRECIO NETO: $' + this.vuelos[item].costoneto + " " + impuesto;
      ...
      document.getElementById('reserva-edit').onsubmit = function() {
        ...
        if (isNaN(d) || d == 0) {
          ...
          if (flag) {
            window.alert("Error: usted ya tiene reservado un viaje para esta fecha");
          }else{
            self.vuelos[item].reservas.push(d);
            window.alert("Pasaje reservado correctamente");
